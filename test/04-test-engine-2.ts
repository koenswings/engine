...
describe(`The Test Master must be able to connect to Test Engine 2 on address ${testEngine2Address} via the appnet ${testNet}`, () => {
    ...
    // it(`Before the connection, the engineSet of the network must not yet contain test engine 2`, async function () {
    //     expect(findEngineByHostname(network2, testEngine2Name)).to.not.exist
    // })

    it('The test machine must be able to make the connection to test engine 2 ', async function () {
        this.timeout(0)
    ...
    it(`must have a wsProvider object for the address ${testEngine2Address}`, async function () {
        if (!network) this.skip()
        expect(network.connections).to.have.property(`${testEngine2Address}:1234`)
        expect(network.connections[`${testEngine2Address}:1234`]).to.exist
      })

      it(`the wsProvider must be in the connected state`, async function () {
        if (!network) this.skip()
        expect(network.connections[`${testEngine2Address}:1234`].wsconnected).to.be.true
      })
    ...
    it('The connection must have delivered a remote engine object ', async function () {
        ...
        // if (Object.keys(network2.appnet.engines).length == 2) {
        ...
        describe(`a disk called ${testDisk2.name}: `, async function () {
            ...
            describe(`that has the apps as described in the test setup: `, async function () {
                it(`It must have the right amount of apps`, async function () {
                    if (!remoteEngine || !disk) this.skip()
                    expect(getApps(store, disk)).to.have.lengthOf(testDisk2.apps.length)
                })

                it(`Each app in the test setup must be present in the disk object`, async function () {
                    if (!remoteEngine || !disk) this.skip()
                    testDisk2.apps.forEach(app => {
