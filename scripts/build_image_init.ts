...
// Getting the offset
const OFFSET = await executeWithErrorHandler(
    `fdisk -l ${argv[1]} | grep W95 | awk '/^[^ ]*1/{ print $2*512 }'`,
    "Failed to get disk offset"
);

// Creating and mounting boot directory
await executeWithErrorHandler(
    `mkdir -p boot`,
    "Failed to create boot directory"
);
await executeWithErrorHandler(
    `sudo mount -o loop,offset=${OFFSET} ${argv[1]} boot`,
    "Failed to mount the boot directory"
);
...
const PASS = await executeWithErrorHandler(
...
await executeWithErrorHandler(
    ...
    "Failed to copy userconf.txt to boot directory"
);
await executeWithErrorHandler(
    `sudo sync`,
    "Failed to synchronize file systems"
);

// Unmounting and removing the boot directory
await executeWithErrorHandler(
    `sudo umount boot`,
    "Failed to unmount the boot directory"
);
await executeWithErrorHandler(
    `rmdir boot`,
    "Failed to remove the boot directory"
);
...
if (is64Bit) {
    await executeWithErrorHandler(
        `./create-image -64 ${argv[1]}`,
        "Failed to execute the create-image script in 64-bit mode"
    );
} else {
    await executeWithErrorHandler(
        `./create-image ${argv[1]}`,
        "Failed to execute the create-image script"
    );
}
